...
if [ "$1" = "logout" ]; then
  userIndex=`curl -s -A "Mozilla/5.0 (Windows NT 10.0; Win64; x64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/61.0.3163.91 Safari/537.36" -I http://10.8.2.2/eportal/redirectortosuccess.jsp | grep -o 'userIndex=.*'`
...
if [ "$#" -lt "3" ]; then
  ...
  echo "interface can be \"campus\", \"chinamobile\", \"chinanet\" and \"chinaunicom\"."
...
loginPageURL=`curl -s "http://10.8.2.2" | awk -F \' '{print $2}'`
